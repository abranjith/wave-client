<!-- Generated by spec-lite v0.0.3 | sub-agent: memorize | updated: 2026-03-05 -->
...
- **Backwards compatibility is not required.** When making code changes or refactors, freely rewrite code without preserving deprecated signatures, legacy behavior, or unused parameters. Remove deprecated code cleanly — do not keep it for compatibility.
...
- **Service placement**: When designing a service, evaluate whether it is needed (or could reasonably be needed) by more than one package (e.g., both `vscode` and `server`). If so, implement it in `packages/shared/src/services/` rather than in a package-specific `src/services/`. Package-specific `src/services/` is reserved for logic that truly has no cross-package utility.
