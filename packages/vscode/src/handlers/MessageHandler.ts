...
export class MessageHandler {
    ...
    private async handleExportCollection(message: any): Promise<void> {
        ...
        try {
            ...
            // Export the collection to Wave JSON format
            // Note: For other formats (Postman, etc.), the webview should transform
            // the collection using @wave-client/core transformers before saving.
            const { content, suggestedFilename } = await collectionService.export(collection);

            // Determine file filter
            const filterLabel = 'JSON Files';
