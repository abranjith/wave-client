...
let securityServiceInstance: ISecurityService | null = null;

/**
 * Interface for SecurityService to avoid circular dependencies
 */
export interface ISecurityService {
    readEncryptedFile<T>(filePath: string, defaultValue: T): Promise<T>;
 ...
 */
export function setSecurityServiceInstance(service: ISecurityService): void {
...
export abstract class BaseStorageService {
    ...
    protected async readJsonFileSecure<T>(filePath: string, defaultValue: T): Promise<T> {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Unknown error';
            console.error(`Error reading secure file ${filePath}:`, message);
            throw error;
        }
    }
    ...
    protected async writeJsonFileSecure<T>(filePath: string, data: T): Promise<void> {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Unknown error';
            console.error(`Error writing secure file ${filePath}:`, message);
            throw error;
        }
    }
}
